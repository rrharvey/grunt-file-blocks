...
var getRegExp = function (template) {
    var pattern = grunt.template.process(template, fileReplace);
...
FileProcessor.prototype.getBlocks = function (configs) {
    ...
    if (block) {
        grunt.warn('Block "' + block.name + '" does not have an endbuild comment.');
    }
...
FileProcessor.prototype.processBlock = function (block) {
    ...
    block.files.forEach(function (file) {
        var line = grunt.template.process(block.template, { data: { file: file } });
